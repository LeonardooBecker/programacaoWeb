...
//Arruma o modal no tamanho da tela
$(document).ready(function() {
    var largura = $("nav").width();
    var margemEsquerda = parseInt($("nav").css("padding-left"));
    var margemDireita = parseInt($("nav").css("padding-right"));
    var larguraTela = $(window).width();

    var tamanhoNav=largura+margemDireita+margemEsquerda;
    $(".modal").css({'left':tamanhoNav,'width':(larguraTela-tamanhoNav)})
});
...
function myFunction(xml) {
    ...
    if (allMatriculas.includes(aluno)) {
        $("#alunoAtual").remove();
        $("#aluno").append("<h4 id='alunoAtual'>Aluno: " + aluno + "</h4>")
        limpaTabela();
    }
    else {
        $("#alunoAtual").remove();
